...
router.delete('/:id', passport.authenticate('jwt', {session: false}), (req,res) => {
  ...
  Profile.findOne({ user: req.user.id })
    ...
    .catch(err => err.status(404).json({postnotfoun: 'No post found'}));
});
...
router.post('/comment/:id', passport.authenticate('jwt', {session: false}), (req,res) => {
  ...
  // we have to check text only so we're using same validator with posts and comments
  const {errors, isValid} = validatePostInput(req.body);
  //check validation
  if(!isValid) {
    // if any errors 400 and err obj
  ...
  Post.findById(req.params.id)
    ...
    .catch(err => res.status(404).json({postnoutfound: 'No post found'}));
});
